...
class employee_data:
    def _init__(self,first_name,last_name,position,year,month,day,graduate):
        ...
        self.read_option = user_option

    def read_option(self):
        while True:
            self.user_option = input("This is a list of your options: add: Add an Employee, remove: Remove an Employee, list: List the Employees ,update: Update Employee Data, exit: Exit the app")
            self.user_option = user_option.strip()

            if self.user_option in ["add", "remove", "update", "list", "exit"]:
